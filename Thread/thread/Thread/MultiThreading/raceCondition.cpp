...
{
	cout<<"void function(int number, char b)\n";
	for(int i=0;i<10;i++)
	{
		auto startTime = std::chrono::high_resolution_clock::now();
		std::this_thread::sleep_for(std::chrono::milliseconds(1919));
...
void function1(Structure * we)
{
	cout<<"void function1(Structure * we)\n";
	for(int i=0;i<10;i++)
	{
		auto startTime = std::chrono::high_resolution_clock::now();
...
int main()
{
	...
	Structure * pt = new Structure();
	thread t2(function1, pt);
	t1.join();
	t2.detach();
