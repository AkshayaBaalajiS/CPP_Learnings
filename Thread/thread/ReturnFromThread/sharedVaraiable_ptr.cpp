...
Both threads can enter the critical section at the same time if we are using only one thread at a time 
...
and if both are having the same amount of sleep 
this result to undefined behaviour 
*/
struct Structure
{
	int * a ;
...
void func(Structure * ptr)
{
	...
	for(int i=0;i<10;i++)
	{
		...
		{
			// lock_guard<mutex>l1(m1);
			// lock_guard<mutex>l2(m2); // deadlock 
...
void func1(Structure * ptr)
{
	...
	for(int i=0;i<10;i++)
	{
		...
		auto endTime = std::chrono::high_resolution_clock::now();
		{
			...
			// lock_guard<mutex>l2(m2);
			// lock_guard<mutex>l1(m1);  // dead lock
			std::scoped_lock s(m1,m2);
			*ptr-=23;
		}
		auto dura = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
