...
void function3(atomic<int> & atovar)
{
	for(int i=0;i<10;i++)
	{
		...
		{
			*atoStr+=atovar.load();
			std::this_thread::sleep_for(std::chrono::milliseconds(1034)); // for makingthe function 4 locxk not acq 
...
void function5()
{
	for(int i=0;i<10;i++)
	{
		...
		{
			*atoStr-=6;
			std::this_thread::sleep_for(std::chrono::milliseconds(1104));
...
{	
	...
	for(int i=0;i<10;i++)
	{
		std::this_thread::sleep_for(std::chrono::milliseconds(1734));
		atoReady = true;
		cv.notify_one();
		auto startTime = std::chrono::high_resolution_clock::now();
		unique_lock<mutex> ul(m1);
